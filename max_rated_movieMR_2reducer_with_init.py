...
class max_rated_movieMR_2reducer_with_init(MRJob):
    def steps(self):
        return [
            MRStep(mapper=self.load_movie_mapper, reducer_init = self.reducer_init,  reducer=self.movie_rating_reducer),
    ...
    def reducer_init(self):
        ...
        with open('C:\python_workspace\Hadoop_Kickstart_Python\data\u.item') as file:
            for line in file:
                fields = line.split('|')
                self.movie_names[fields[0]] = fields[1].decode( 'UTF-8', 'ignore')
    ...
    def movie_rating_reducer(self, movie_id, count):
        ...
        yield None, (sum(count), self.movie_names[movie_id])
...
if __name__ == '__main__':
    max_rated_movieMR_2reducer_with_init.run()